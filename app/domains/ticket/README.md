...
## Documentação Detalhada da Rota de Criação

### O que a rota espera receber

`CreateTicketDTO`:

| Campo | Tipo no código | Obrigatório | Descrição |
| --- | --- | --- | --- |
| `triage_id` | `PydanticObjectId` | Sim | Identificador da triagem/origem do transbordo |
| `type` | `TicketType` | Sim | Tipo funcional do ticket |
| `criticality` | `TicketCriticality` | Sim | Nível de criticidade |
| `product` | `str` | Sim | Produto ou sistema afetado |
| `description` | `str` | Sim | Descrição funcional do problema |
| `chat_ids` | `list[PydanticObjectId]` | Sim | IDs de chats relacionados |
| `client_id` | `UUID` | Sim | Identificador do cliente no domínio `auth` |

### Comportamento confirmado pelo código

Ao criar o ticket, o service:
- busca o cliente usando `UserService.get_by_id(client_id)`
- monta `TicketClient`
- define `status = TicketStatus.OPEN`
- define `creation_date = datetime.now(UTC)`
- define `comments = []`
- define `agent_history = []`
- persiste o documento no MongoDB

### Como o client é resolvido e montado

Comportamento confirmado:
- o `client_id` é resolvido no domínio `auth`
- a busca reutiliza `UserService.get_by_id(...)`
- se o usuário não existir, a criação falha com `404`

Comportamento inferido do código:
- o projeto não possui hoje um domínio próprio para empresa/cliente corporativo
- por isso, `TicketCompany` é montado internamente com:
  - `id = user.id`
  - `name = f"{client_name} account"`

Isso é uma limitação técnica atual do projeto, não um contrato externo formal de negócio.
...
```json
{
  ...
  "title": "HTTP Error",
  "status": 404,
  "detail": "Client 0f7d7c4f-7b5b-45cb-9d85-6f3c69f0b5d2 does not exist.",
  "instance": "/api/tickets/",
  "meta": {
    "timestamp": "2026-04-01T20:22:00.200000+00:00",
    "success": false,
    "request_id": "..."
  }
}
...
## Documentação Detalhada da Rota de Atualização de Status

### Path param

| Campo | Tipo no código | Formato esperado |
| --- | --- | --- |
| `ticket_id` | `PydanticObjectId` | ObjectId de 24 caracteres hexadecimais |

Exemplo válido:
- `67f0ca60e4b0b1a2c3d4e601`
...
Transições válidas confirmadas pelo código:
...
- atualizar para o mesmo status retorna `400`
- ticket inexistente retorna `404`
- transição fora da tabela retorna `400`
...
aplicada em um ticket atualmente `open`.
...
- O domínio `ticket` já implementa criação e atualização de status.
- A criação resolve o cliente pelo domínio `auth`, não por um domínio dedicado de clientes.
- Não existe hoje uma fonte explícita de `company` no projeto fora do próprio model de `ticket`.
- Por isso, a montagem de `TicketCompany` no service é uma solução de compatibilidade interna e deve ser revisitada se surgir um domínio real de empresas/clientes.
- O README descreve apenas comportamentos confirmados pelo código atual.
- Não há, neste domínio, endpoints de listagem, leitura individual, comentários ou atribuição de agente.
- Não há tratamento específico de erros de persistência no repository além da propagação da exceção.
